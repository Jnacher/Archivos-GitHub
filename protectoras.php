...
if(isset($_POST['actualizar']) && isset($_POST['provincia'])){
	...
	$sqlPro="select telefono, nombreProtectora, webProtectora from usuarios where webProtectora!='' AND nombreProtectora!=''".$porProv;
	if($reAd=$db->GetRow($sqlPro)){
	  $infoProtectoras.='<div class="cabeceraProtectorasSub">
									<div class="uno">'.$reAd['nombreProtectora'].'</div>
									<div class="dos">'.$reAd['telefono'].'</div>
									<div class="tres">'.$reAd['webProtectora'].'</div>
