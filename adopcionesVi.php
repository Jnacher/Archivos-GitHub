...
if(isset($_POST['actualizar'])){
  ...
  $sqlAdopciones="select * from anuncios where tipo='a' ".$sql;
}else{
  $sqlAdopciones="select * from anuncios where idCategoria=1 AND tipo='a'";
}//$db->debug=true;
$anunciosHtml='';
if($resAd=$db->GetAll($sqlAdopciones)){
  foreach($resAd as $reAd){
    ...
    $anunciosHtml.='<div class="anunciosIndi">
			...
			$sqlImg= "select * from imagenes where idAnuncio=".$reAd['anuncioId'];
				if($resImgs=$db->GetAll($sqlImg)){
					...
					$anunciosHtml.='<div class="anunciosImagen"><a href="verAnuncio.php?idAnuncio='.$reAd['anuncioId'].'">
								<img src="images/grandes/'.$resImgs[0]['rutaImg'].'" width="100" height="100" /></a></div>
